...
func (cs *State) CreateScalarisBlock(txs *[]types.Tx, client client.Client) (*types.Block, error) {
	...
	ret, err := cs.blockExec.CreateScalarisProposalBlock(cs.Height, cs.state, lastExtCommit, proposerAddr, txs)
...
func (cs *State) tryAddVotes(votes []types.Vote) {
	...
	for idx := range votes {
		cs.Votes.AddVote(&votes[idx], p2p.ID(votes[idx].ValidatorAddress.String()), extEnabled)
	}
}
